...
/**
 * Life Data, told as the narrative climax — a gradual reveal from a single
 * observation to a living atlas, rather than a dashboard of tools. The
 * exploration tools (taxonomy / map / insights) appear last, as a quiet
 * "continue exploring", never as the headline.
 */
export function LifeDataHub() {
  ...
  return (
    <PageTransition>
      <div className="bg-ivory pb-24 pt-[72px]">
        {/* ===== Opening ===== */}
        <header className="container-narrow py-24 md:py-36">
          <Reveal>
            <p className="eyebrow text-ochre">{t('lifeData.narrative.eyebrow')}</p>
            <h1 className="headline mt-4 text-4xl text-charcoal md:text-6xl">
              {t('lifeData.narrative.title')}
            </h1>
            <p className="mt-6 max-w-2xl text-pretty leading-cn text-lg text-charcoal-soft">
              {t('lifeData.narrative.body')}
            </p>
          </Reveal>
        </header>

        {/* ===== Beat 1 — an observation ===== */}
        <section className="container-narrow pb-24 md:pb-32">
          <Reveal>
            <div className="overflow-hidden rounded-[20px] shadow-[0_40px_80px_-34px_rgba(38,36,31,0.45)]">
              <img
          ...
          <Reveal delay={0.1} className="mx-auto mt-8 max-w-xl text-center">
            <p className="eyebrow text-forest-mist">{t('lifeData.narrative.observation')}</p>
            <h2 className="headline mt-3 text-2xl text-charcoal md:text-3xl">
              {t('lifeData.narrative.observationTitle')}
            </h2>
            <p className="mt-3 text-pretty leading-cn text-charcoal-soft">
              {t('lifeData.narrative.observationBody')}
            </p>
          </Reveal>
        </section>
        ...
        <section className="border-y border-stone-light/50 bg-ivory-50 py-24 md:py-32">
          <div className="container-wide">
            <Reveal className="mx-auto max-w-xl text-center">
              <p className="eyebrow text-forest-mist">{t('lifeData.narrative.species')}</p>
              <h2 className="headline mt-3 text-3xl text-charcoal md:text-4xl">
                {t('lifeData.narrative.speciesTitle')}
              </h2>
              <p className="mt-3 text-pretty leading-cn text-charcoal-soft">
                {t('lifeData.narrative.speciesBody')}
              </p>
            </Reveal>
            ...
            <div className="mx-auto mt-14 grid max-w-4xl gap-6 sm:grid-cols-3">
              {featuredLoading
                ...
                : (featured ?? []).slice(0, 3).map((row, i) => (
                    <Reveal key={row.taxon.id} delay={i * 0.08} className="group">
                      <Link
        ...
        {/* ===== Beat 3 — the tree of life ===== */}
        <section className="bg-forest-deep py-24 text-ivory-50 md:py-32">
          <div className="container-wide">
            <Reveal className="mx-auto max-w-2xl text-center">
              <p className="eyebrow text-sage">{t('lifeData.narrative.taxon')}</p>
              <h2 className="headline mt-3 text-3xl text-ivory-50 md:text-5xl">
                {t('lifeData.narrative.treeTitle')}
              </h2>
              <p className="mt-4 text-pretty leading-cn text-ivory-50/70">
                {t('lifeData.narrative.treeBody')}
              </p>
            </Reveal>
        ...
        {/* ===== Beat 4 — a living atlas ===== */}
        <section className="container-wide py-24 md:py-32">
          <Reveal className="mx-auto max-w-2xl text-center">
            <p className="eyebrow text-forest-mist">{t('lifeData.narrative.atlas')}</p>
            <h2 className="headline mt-3 text-3xl text-charcoal md:text-5xl">
              {t('lifeData.narrative.atlasTitle')}
            </h2>
            <p className="mt-4 text-pretty leading-cn text-charcoal-soft">
              {t('lifeData.narrative.atlasBody')}
            </p>
          </Reveal>
          ...
          {/* continue exploring — the tools, quietly */}
          <Reveal className="mt-16">
            <p className="text-center text-xs uppercase tracking-widest-2 text-forest-mist">
              {t('lifeData.narrative.continue')}
            </p>
...
function StatBlock({
  ...
  return (
    <Reveal className="px-2 text-center">
      ...
      <p className="mt-2 font-display text-base font-medium text-charcoal">{label}</p>
